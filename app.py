...
tinyfish_service = TinyfishService(
    ...
    base_url=config['tinyfish'].get('base_url', 'https://agent.tinyfish.ai/v1'),
    timeout=config['tinyfish'].get('timeout', 60),
    browser_profile=config['tinyfish'].get('browser_profile', 'lite'),
    proxy_config=config['tinyfish'].get('proxy_config')
)
