...
class ResearchOrchestrator:
    ...
    def gather_field_data(self, session_id: str) -> Dict[str, Any]:
        ...
        source_plan = self.openai.plan_source_extraction(
            user_query=session.breakdown.user_request or session.field_context.field_name,
            field_name=session.field_context.field_name,
            user_background=session.breakdown.user_context,
        )

        sources = source_plan.get("sources", [])
        if not sources:
            return {"error": "OpenAI did not return any sources to extract from."}

        extracted_sources = self.tinyfish.extract_from_sources(sources)
        raw_data = self.tinyfish.build_research_corpus(sources, extracted_sources)

        session.field_context.raw_data = raw_data
        session.field_context.search_queries = source_plan.get("search_queries", [])
        session.field_context.sources = [
            {
            ...
            for source, extraction in zip(sources, extracted_sources)
        ]

        return {
            "raw_data_collected": len(raw_data),
            "search_queries": session.field_context.search_queries,
            "sources": session.field_context.sources,
        }
    ...
    def generate_field_context(self, session_id: str) -> FieldContext:
        ...
        prior_context = session.field_context

        field_context = self.openai.generate_field_context(
            prior_context.raw_data,
            prior_context.field_name
        )

        field_context.raw_data = prior_context.raw_data
        field_context.search_queries = prior_context.search_queries
        field_context.sources = prior_context.sources or field_context.sources
    ...
    def full_session_flow(self, session_id: str, user_query: str, user_background: str = "") -> Dict[str, Any]:
        ...
        gather_result = self.gather_field_data(session_id)
        if gather_result.get("error"):
            return {"error": gather_result["error"]}
